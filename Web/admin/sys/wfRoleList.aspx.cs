...
namespace Web.admin.sys
{
    public partial class wfRoleList : BasePage
    {
        ...
        {
            int rId = ConvertInt32(this.hfRoleId.Value, 0);
            if (rId == 0)
            {
                Add();
                InitDataClear();
        ...
        protected void btnDelete_Click(object sender, EventArgs e)
        {
            ...
            {
                string rId = MstGridView.DataKeys[gvr.RowIndex].Value.ToString();
                CheckBox chk = (CheckBox)gvr.FindControl("chkItem");
                if (chk.Checked)
                {
                    ZHY.BLL.Role bll = new ZHY.BLL.Role();
                    bll.Delete(ConvertInt32(rId, 0));
                }
            }
            MstGridViewBind();
        ...
        {
            ZHY.BLL.Role bll = new ZHY.BLL.Role();
            ZHY.Model.Role model = bll.GetModel(ID);
            if (model != null)
            {
                model.RoleName = this.txtRoleName.Text;
                model.RoleDes = this.txtRoleDes.Text;
        ...
        {
            ZHY.BLL.Role bll = new ZHY.BLL.Role();
            ZHY.Model.Role model = bll.GetModel(rId);
            this.hfRoleId.Value = model.RoleID.ToString();
