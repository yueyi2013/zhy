...
{
    public partial class wfCommentList : BasePage
    {
        ...
        {
            ZHY.BLL.ArticleComment bll = new ZHY.BLL.ArticleComment();
        ...
        {
            int ntId = ConvertInt32(this.hfACMId.Value, 0);
            if (ntId == 0)
            {
                Add();
                InitDataClear();
        ...
        protected void btnDelete_Click(object sender, EventArgs e)
        {
            ...
            {
                string acmId = MstGridView.DataKeys[gvr.RowIndex].Value.ToString();
                CheckBox chk = (CheckBox)gvr.FindControl("chkItem");
                if (chk.Checked)
                {
                    ZHY.BLL.ArticleComment bll = new ZHY.BLL.ArticleComment();
                    bll.Delete(ConvertInt32(acmId, 0));
                }
            }
            MstGridViewBind();
        ...
        {
            ZHY.BLL.ArticleComment bll = new ZHY.BLL.ArticleComment();
            ZHY.Model.ArticleComment model = bll.GetModel(ID);
            if (model != null)
            {
                model.ACMStatus = this.rbStatus.SelectedValue;
                model.ACMContent = this.ftContent.Text;
                ZHY.Model.User user = getLoginUser();
                model.UpdateBy = user.UserName;
        ...
        {
            this.hfACMId.Value = "0";
            this.ftContent.Text = "";
            this.rbStatus.SelectedValue = "A";
        }
        ...
        {
            ZHY.BLL.ArticleComment bll = new ZHY.BLL.ArticleComment();
            ZHY.Model.ArticleComment model = bll.GetModel(rssId);
            this.hfACMId.Value = model.ACMId.ToString();
            this.ftContent.Text = model.ACMContent;
            this.rbStatus.SelectedValue = model.ACMStatus;
        }
        #endregion
    }
}