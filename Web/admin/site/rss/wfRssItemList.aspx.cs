...
namespace Web.admin.site.rss
{
    public partial class wfRssItemList:BasePage
    {
        ...
        {
            int rssId = ConvertInt32(this.hfFeedID.Value, 0);
            if (rssId == 0)
            {
                Add();
                InitDataClear();
        ...
        protected void btnDelete_Click(object sender, EventArgs e)
        {
            ...
            {
                string feedId = MstGridView.DataKeys[gvr.RowIndex].Value.ToString();
                CheckBox chk = (CheckBox)gvr.FindControl("chkItem");
                if (chk.Checked)
                {
                    ZHY.BLL.RSSChannelItem bll = new ZHY.BLL.RSSChannelItem();
                    bll.Delete(ConvertInt32(feedId, 0));
                }
            }
            MstGridViewBind();
        ...
        protected void btnModify_Click(object sender, EventArgs e)
        {
            foreach (GridViewRow gvr in MstGridView.Rows)
            {
                ...
                if (chk.Checked)
                {
                    InitData(ConvertInt32(feedId, 0));
                    this.mpMstAdd.Show();
        ...
        {
            ZHY.BLL.RSSChannelItem bll = new ZHY.BLL.RSSChannelItem();
            ZHY.Model.RSSChannelItem model = bll.GetModel(ID);
            if (model != null)
            {
                model.RCItemTitle = this.txtFeedTitle.Text;
                model.RCItemAuthor = this.txtFeedAuthor.Text;
        ...
        {
            this.hfFeedID.Value = "0";
            this.txtFeedTitle.Text = "";
            this.txtFeedAuthor.Text = "";
            this.txtFeedComment.Text = "";
            this.ftContent.Text = "";
        }
        ...
        {
            ZHY.BLL.RSSChannelItem bll = new ZHY.BLL.RSSChannelItem();
            ZHY.Model.RSSChannelItem model = bll.GetModel(rssId);
            this.hfFeedID.Value = model.RCItemId.ToString();
